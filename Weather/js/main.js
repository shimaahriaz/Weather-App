...
window.onload = async function() {
  countery = "Cairo";
  await getAllNews(countery);
};

async function getAllNews(list) {
  if (list == null || list.trim() === "") {
    ...
    try {
      const response = await fetch(
        `https://api.weatherapi.com/v1/forecast.json?key=568b6ac4c66f4d308fc164821232611&q=${list}$2&days=3#`
      );
...
inputElement.addEventListener("input", function() {
  countery = inputElement.value.trim();
  (async function() {
    await getAllNews(countery);
  })();
});
